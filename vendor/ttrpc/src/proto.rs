...
impl GenMessage {
    ...
    pub async fn read_from(
        ...
        // Idle RPC connections and long-poll methods may remain silent indefinitely.
        // Once a peer begins a frame, however, its entire header/body has a deadline.
        ...
        let read = async {
            ...
            // Never drain a peer-selected oversized body before reporting failure.
            ...
            reader
                ...
                .await
                .map_err(|e| Error::Socket(e.to_string()))?;
            Ok::<_, Error>((header, content))
        };
        let (header, content) = tokio::time::timeout(std::time::Duration::from_secs(30), read)
            .await
            .map_err(|_| Error::Socket("RPC frame deadline exceeded".into()))??;
    ...
    C::E: std::fmt::Display,
{
    ...
    /// Decodes a MessageHeader from reader.
    pub async fn read_from(reader: impl tokio::io::AsyncReadExt + Unpin) -> TtResult<Self> {
...
mod tests {
    ...
    async fn async_gen_message() {
        // Oversized headers terminate the connection without draining a body.
        assert!(matches!(
            GenMessage::read_from(&MESSAGE_HEADER[..]).await,
            Err(GenMessageError::InternalError(Error::RpcStatus(_)))
        ));
    ...
    async fn async_message() {
        assert!(Message::<Request>::read_from(&MESSAGE_HEADER[..])
            .await
            .is_err());
...
mod kata_deadline_tests {
    use super::*;
    use tokio::io::AsyncWriteExt;
    #[tokio::test(start_paused = true)]
    async fn partial_frames_expire_but_idle_connections_do_not() {
        let (mut guest, host) = tokio::io::duplex(64);
        let task = tokio::spawn(async move { GenMessage::read_from(host).await });
